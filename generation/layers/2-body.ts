import { getRandomIntInclusive } from "./common";

export async function getBody() {
  const svgOpenTag = `<g id="Body">`;
  const svgCloseTag = `</g>`;

  const bodyOptions = [
    `<rect x="306.76" y="707.8" width="414.66" height="319.69" style="fill:#4e3049" />
    <path d="M211,1022.23H140v-90c0-94,76.49-170.5,170.5-170.5v71a99.61,99.61,0,0,0-99.5,99.5Z" transform="translate(4.96 5.26)" style="fill:#4e3049" />
    <path d="M877.46,1022.23h-71v-90a99.61,99.61,0,0,0-99.5-99.5v-71c94,0,170.5,76.48,170.5,170.5Z" transform="translate(4.96 5.26)" style="fill:#4e3049" />
    <rect x="279.42" y="757.49" width="45" height="108" style="fill:#4b2955" />
    <rect x="702.42" y="757.49" width="45" height="108" style="fill:#4b2955" />`,
    `<rect x="143" y="482.84" width="723.36" height="552.16" style="fill:#bb3f26" />
    <rect x="298.87" y="764.65" width="402.04" height="270.35" style="fill:#553060" />
    <rect x="499.69" y="684.89" width="204.71" height="350.11" style="fill:#172028" />`,
  ];

  const index = getRandomIntInclusive(0, bodyOptions.length - 1);
  console.log(`  Body: ${index}`);

  const bodySVG = `${svgOpenTag}${bodyOptions[index]}${svgCloseTag}`;

  return bodySVG;
}
